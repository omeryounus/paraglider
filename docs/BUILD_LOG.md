...
## Session 7: The Canopy Identity Fix — Paraglider Wing, Not a Dome Parachute
- **Feedback chain**: "what happened to parachute" → answered (renders, with pixel evidence) → "this is not the correct parachute render" → found `paintStudioCanopy` painting flat bands over the GLB's real PBR textures → fixed (`3f5004e`, real wine-red fabric shown) → "still not correct" → geometry analysis → **"this is better"**.
- **Root cause (the real one)**: the studio `parachute.glb` is a round DOME parachute — 2.09:1 square footprint, 1.8 m-tall volume with suspension lines — a skydiving canopy. This is a PARAGLIDER game (seated harness, brake toggles, weight shift, big-ears). The correct canopy is the procedural ram-air wing that `attachStudioAssets` had been HIDING under the dome since the initial commit: NACA-cambered double-surface airfoil with open intakes, elliptical plan, anhedral arc, rib tapes, ripstop normal map, ruby-red panels with wine tips, brake/big-ears deformation (`createAirfoil`/`panelColor`/`deformCanopy`).
- **Fix (`d57add6`)**: `attachStudioAssets` no longer loads the dome at all — only the PILOT (mixamo rig + gear placement; person/pilot GLB fallbacks intact). Dead code removed: `makeCanopyFabric`, `paintStudioCanopy`, the dome mount block. `deformStudioCanopy` no-ops without a studio canopy; `deformCanopy` drives the wing as before.
- **Pack cleanup**: `models/parachute.glb` (1.44 MB) removed from the contest staging list — dead weight; the wing is procedural. Zip: 37 entries, 6.34 MB.
- **Verification**: judge-path run on the final zip (portrait 405×720, fresh unzip + local server): boots, launches, flies (AGL 67 / 58 km/h), ZERO console errors, ZERO 404s, `parachute.glb` never fetched (network trace), wing renders as a wide thin red band spanning 61% of the frame — the elongated ram-air silhouette (the dome showed as a fat blob). Live Vercel site verified identical (dome fetch count 0, same wing band, same flight state). tsc clean, 27/27 vitest, site build OK.
